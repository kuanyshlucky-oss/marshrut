...
const LIBRARY_CONSPECTS = {
  ...
  // собрана из уже проверенных conspect-полей вопросов этого же банка.
  M107: {
...
• Определитель матрицы 2×2: |a b; c d| = ad − bc.
• Минор Mᵢⱼ — определитель матрицы, полученной вычёркиванием i-й строки и j-го столбца исходной матрицы.
• Алгебраическое дополнение: Aᵢⱼ = (−1)^(i+j)·Mᵢⱼ.
• Разложение определителя по строке/столбцу: det A = Σ aᵢⱼ·Aᵢⱼ.
• Действия с матрицами одинакового размера (сложение/вычитание) выполняются поэлементно.
• Обратная матрица: A⁻¹ = (1/det A)·adj(A), где adj(A) — присоединённая (союзная) матрица из алгебраических дополнений; A·A⁻¹=E; существует только при det A ≠ 0.
• Ранг матрицы находится методом элементарных преобразований (метод Гаусса) — приведением к ступенчатому виду.
## Аналитическая геометрия
• Абсцисса точки — её первая координата (x); ордината — вторая (y); аппликата — третья (z, в пространстве).
• Каноническое уравнение эллипса с центром в начале координат и полуосями a, b: x²/a² + y²/b² = 1.` },
          { title: 'Дифференциальные и интегральные исчисления функции одной переменной', body:
`## Пределы
• Предел рациональной функции при x→∞ определяется сравнением старших степеней числителя и знаменателя: степень числителя больше — предел бесконечен, меньше — предел равен 0, равны — предел равен отношению коэффициентов при старших степенях.
• Эквивалентности бесконечно малых при x→0: sin(kx) ~ kx, tg(kx) ~ kx — удобны для вычисления пределов отношений заменой на эквивалентную величину.
## Производная и дифференциал
• Геометрический смысл производной: угловой коэффициент касательной к графику в точке x₀ равен f'(x₀).
• Производная сложной функции (цепное правило): (f(g(x)))' = f'(g(x))·g'(x).
• Таблица производных: (xⁿ)'=nxⁿ⁻¹, (√x)'=1/(2√x), (aˣ)'=aˣ·ln a, (eˣ)'=eˣ, (sin x)'=cos x, (cos x)'=−sin x, (ln x)'=1/x.
• Дифференциал функции: df = f'(x)dx.
## Первообразная и определённый интеграл
• Формула Ньютона—Лейбница: ∫[a,b] f(x)dx = F(b) − F(a), где F — первообразная f.
• Таблица первообразных: ∫xⁿdx=xⁿ⁺¹/(n+1)+C, ∫dx/x=ln|x|+C, ∫eˣdx=eˣ+C, ∫sin x dx=−cos x+C, ∫cos x dx=sin x+C.
## Дифференциальные уравнения первого порядка
• Порядок уравнения определяется по старшей производной (или дифференциалу) в его записи.
• С разделяющимися переменными: приводится к f(y)dy = g(x)dx и интегрируется почленно; при делении на выражение с y отдельно проверяют, не потеряно ли решение (например, y=0).
• Линейное уравнение: y' + p(x)y = q(x). Интегрирующий множитель μ(x)=e^(∫p(x)dx) сводит уравнение к (y·μ)' = q·μ, откуда y·μ = ∫q·μ dx + C.
• Уравнение Бернулли: y' + p(x)y = q(x)yⁿ. Заменой z = y^(1−n) сводится к линейному относительно z.
• Однородное уравнение: правая часть — функция только отношения y/x. Заменой y = vx (v=v(x)) сводится к уравнению с разделяющимися переменными относительно v.
• Уравнение Клеро: y = xy' + f(y'). Общее решение — заменой y' на произвольную постоянную C: y = Cx + f(C).
• Если уравнение неудобно решать относительно y(x), можно взять x как функцию y: dx/dy = 1/y' — иногда именно так оно оказывается линейным относительно x(y).
• Задача Коши — нахождение частного решения по начальному условию: подстановкой находят константу C.
• Линейное однородное уравнение с постоянными коэффициентами (второй порядок): характеристическое уравнение получают заменой y''→k², y'→k, y→1.` },
          { title: 'Дифференциальные и интегральные исчисления функции многих переменных', body:
`## Частные производные
• Частная производная по одной переменной находится дифференцированием по ней при условии, что все остальные переменные считаются постоянными.
• Частные производные высших порядков находятся повторным дифференцированием: ∂²f/∂x² — вторая частная производная по x; смешанная ∂²f/∂x∂y — сначала по одной переменной, затем по другой.
• Дифференцирование сложной функции (композиции) выполняется по цепному правилу для каждой промежуточной переменной.
## Кратные интегралы
• Двойной/тройной интеграл по прямоугольной (кубической) области вычисляется последовательным (повторным) интегрированием — сначала по внутренней переменной при фиксированных остальных, затем по следующей, и так до внешней.
• Порядок интегрирования (по x, y, z или в другой последовательности) определяется пределами области и записью самого интеграла.
## Уравнения в частных производных
• Проверка, является ли функция решением уравнения в частных производных, выполняется прямой подстановкой её частных производных нужного порядка в уравнение.` },
        ],
      },
      {
        key: 'fizika',
...
• Закон Ньютона: F = G·m₁m₂/r² — сила притяжения прямо пропорциональна произведению масс и обратно пропорциональна квадрату расстояния между ними.
• Напряжённость гравитационного поля (ускорение свободного падения) у поверхности тела: g = GM/R², где M — масса тела, R — его радиус.
• Обобщённый закон Галилея (принцип эквивалентности): в одном и том же поле тяготения все тела падают с одинаковым ускорением независимо от массы.
• Потенциал поля тяготения — потенциальная энергия единичной массы в данной точке поля.
• Работа силы тяготения при перемещении тела массой m между точками на расстояниях r₁ и r₂ от центра: A₁₂ = GMm(1/r₁ − 1/r₂) = mgR²(1/r₁ − 1/r₂), где g и R — значения вблизи поверхности.
• Закон сохранения механической энергии в гравитационном поле: v²/2 − GM/r = const.
• Точка нулевой напряжённости поля между телами массами M₁ и M₂ на расстоянии R: GM₁/x² = GM₂/(R−x)².
## Космические скорости
• Первая космическая скорость (круговая орбита у поверхности): v₁ = √(gR) = √(GM/R) ≈ 7,9 км/с.
• Скорость спутника на круговой орбите радиуса r: v = √(GM/r) — из равенства силы тяготения и центростремительной силы.
• Вторая космическая (параболическая) скорость: v₂ = v₁·√2 — минимальная скорость, чтобы тело покинуло притягивающий центр по незамкнутой траектории (e=1).
• Третья космическая скорость — минимальная скорость у поверхности Земли, чтобы тело покинуло пределы Солнечной системы, преодолев притяжение и Земли, и Солнца.
## Законы Кеплера
• Первый закон: все планеты движутся по эллиптическим орбитам, в одном из фокусов которых находится Солнце.
• Второй закон (закон площадей): радиус-вектор планеты за равные промежутки времени заметает равные площади; секториальная скорость σ = dS/dt = L/(2m) = const (следствие сохранения момента импульса).
• Из второго закона: линейная скорость тела максимальна в перигелии и минимальна в афелии.
• Третий закон: T₁²/T₂² = a₁³/a₂³ — квадраты периодов обращения планет относятся как кубы больших полуосей их орбит.
## Элементы и типы орбит
• Тип орбиты определяется эксцентриситетом e: e=0 — окружность, 0<e<1 — эллипс, e=1 — парабола, e>1 — гипербола.
• Перигелий/афелий — ближайшая/наиболее удалённая от Солнца точки орбиты; для Земли как центра притяжения — перигей/апогей; в общем случае — перицентр/апоцентр.
• Линия апсид — прямая вдоль большой полуоси орбиты, соединяющая перицентр и апоцентр.
• Наклон орбиты i — угол между плоскостью орбиты и плоскостью эклиптики (или другой опорной плоскостью).
• Шесть кеплеровских элементов орбиты: Ω (долгота восходящего узла), i (наклонение), a или p (большая полуось/параметр), e (эксцентриситет), ω (аргумент перицентра), τ (момент прохождения перицентра).
• Уравнение орбиты через фокальный параметр: r = p/(1+e·cosθ), где p = L²/(m|β|) для силы F = β/r².
• Радиус-вектор через эксцентрическую аномалию E: r = a(1 − e·cosE); средняя аномалия M = n(t−τ), n = √(μ/a³).
• Уравнение Кеплера: E − e·sinE = M — связывает эксцентрическую и среднюю аномалии, решается численно (итерациями). Для параболы (e=1) — аналог, уравнение Баркера.
## Задача двух тел и интегралы движения
• Интеграл энергии: h = v²/2 − μ/r = const; определяет тип траектории: h=0 — парабола, h<0 — эллипс, h>0 — гипербола.
• Интеграл площадей (секториальная постоянная): c = r×v.
• Вектор Лапласа (Рунге—Ленца): f = v×c − μr/r, перпендикулярен моменту импульса c (поэтому c·f=0).
• Связь интегралов движения: f² = μ² + h·c², откуда эксцентриситет орбиты e = √(1 + hc²/μ²).
• Барицентр (центр масс) системы при отсутствии внешних сил движется равномерно и прямолинейно.
## Периоды и историческая справка
• Сидерический период — оборот тела относительно неподвижных звёзд; синодический — относительно другого движущегося тела (например, Земли).
• Птолемеева геоцентрическая система: Земля неподвижна в центре, планеты движутся по эпициклам, центры которых движутся по деферентам вокруг Земли.` },
          { title: 'Физические основы механики', body:
`## Кинематика
• Средняя скорость: vср = Δr/Δt.
• Равноускоренное движение (векторная форма): r(t) = r₀ + v₀t + at²/2; путь из состояния покоя: s = at²/2.
• Полное ускорение: тангенциальное aτ = dv/dt (меняет модуль скорости) и нормальное an = v²/R (меняет направление, направлено к центру кривизны).
• Угловое ускорение: ε = d²φ/dt² = dω/dt.
## Законы Ньютона
• Первый закон (инерция): тело сохраняет покой или равномерное прямолинейное движение без внешних воздействий; постулирует существование инерциальных систем отсчёта.
• Второй закон: F = ma = dp/dt, где p = mv — импульс тела.
• Третий закон: F₁₂ = −F₂₁.
• Условие равновесия тела: сумма проекций всех сил на любое направление равна нулю.
## Работа, энергия, удар
• Кинетическая энергия: T = mv²/2.
• Работа в поле тяжести равна изменению потенциальной энергии: A = mgh.
• Закон сохранения механической энергии (без сопротивления воздуха): E = mv²/2 + mgh = const.
• КПД неупругого удара: η = m₂/(m₁+m₂), где m₁ — масса ударяющего тела.
## Трение и наклонная плоскость
• Сила трения скольжения: замедление a = −μg, где μ — коэффициент трения.
• Ускорение на гладкой наклонной плоскости под углом α (без трения): a = g·sinα.
• Виды трения: покоя, скольжения, качения.
## Моменты, рычаг, вращение
• Момент силы: M = F·d, где d — плечо силы.
• Правило моментов (равновесие рычага): F₁·l₁ = F₂·l₂.
• Основное уравнение динамики вращения: M = Iε, где I — момент инерции (для диска I = mr²/2).
• Момент, закручивающий стержень на угол φ: M = Cφ (аналог закона Гука для вращения).
## Деформации
• Закон Гука: Fупр = −kx.
• Механическое напряжение: σ = F/S = E·(Δl/l), где E — модуль Юнга.
• Относительная деформация: ε = x/l.
## Законы сохранения
• Сохранение импульса — следствие однородности пространства; момента импульса — изотропности пространства; энергии — однородности времени (теорема Нётер).
## Системы тел и переменной массы
• Закон движения центра масс: m·aцм = ΣFвнеш.
• Уравнение Мещерского: m·dv/dt = Fвнеш + u·dm/dt, где u — скорость истечения массы относительно тела.
• Тележка и груз через блок: для тележки T = Ma, для груза mg − T = ma.` },
          { title: 'Электродинамика', body:
`## Поля точечных и распределённых зарядов
• Поле вне заряженной сферы (r≥R): E = Q/(4πε₀r²) — как от точечного заряда в центре; внутри сферы (r<R): E=0.
• Поле заряженного шара внутри него (r<R): E = (1/4πε₀)·Qr/R³ — растёт линейно с расстоянием от центра.
• Поле бесконечной плоскости: E = σ/(2ε₀); между двумя разноимённо заряженными плоскостями (конденсатор): E = σ/ε₀.
• Электрический диполь: заряды ±q на расстоянии l, дипольный момент p = Ql. В неоднородном поле на диполь действуют и момент сил, и результирующая сила.
## Диэлектрики и проводники
• Проводники — заряд свободно перемещается по объёму; диэлектрики — свободных зарядов практически нет.
• Электронная поляризация — смещение электронных орбит атомов под действием поля с возникновением индуцированного момента.
• Вектор электрического смещения: D = εε₀E.
• Теорема Гаусса для D: поток через замкнутую поверхность равен сумме свободных зарядов внутри неё.
## Конденсаторы
• Ёмкость: C = q/U; энергия: W = CU²/2.
## Постоянный ток
• Заряд через ток: q = I·t.
• Обобщённый закон Ома для участка цепи: IR = Δφ + Σε.
• Правила Кирхгофа: для узла ΣIₖ=0, для контура ΣIₖRₖ=ΣEₖ.
• Мост Уинстона — измерительная схема для точного измерения неизвестного сопротивления.
• Законы Фарадея для электролиза: m = kIt; электрохимические эквиваленты пропорциональны химическим.
## Магнитное поле
• Сила Лоренца: F = qv×B (при перпендикулярности F=|q|vB).
• Параллельные токи одного направления притягиваются, противоположного — отталкиваются.
• Энергия магнитного поля тока: W = LI²/2 = Φ²/2L, где Φ=LI.
• Парамагнетики намагничиваются по полю (μ>1), диамагнетики — против (μ<1), ферромагнетики — сильно и нелинейно, с остаточной намагниченностью.
## Переменный ток
• Импедансы: резистор Z_R=R, катушка Z_L=iωL, конденсатор Z_C=1/(iωC).
• Полное сопротивление: Z = √(R²+(ωL−1/ωC)²); закон Ома в комплексной форме U=IZ.` },
          { title: 'Физика колебаний и волн', body:
`## Колебательный контур
• Закрытый контур: электрическое поле в конденсаторе, магнитное — в катушке. Открытый контур (антенна): поля распределены в пространстве — излучение электромагнитных волн.
• Реактивные сопротивления: конденсатор Rc=1/(ωC), катушка RL=ωL; полный импеданс Z=√(R²+(ωL−1/ωC)²).
• Закон Ома для контура: Um=Im·√(L/C), где √(L/C) — волновое сопротивление.
## Электромагнитные волны
• Электромагнитная волна — взаимосвязанные колебания электрического и магнитного полей.
• Волны поперечны: E и B колеблются перпендикулярно друг другу и направлению распространения.
• Скорость в вакууме: c = 1/√(ε₀μ₀).` },
          { title: 'Квантовая природа электромагнитного излучения', body:
`## Фотон и энергия излучения
• Энергия фотона: E = hν = hc/λ, где h — постоянная Планка, ν — частота, λ — длина волны.
• Импульс фотона: p = h/λ = E/c.
• Уравнение Эйнштейна для фотоэффекта: hν = Aвых + Eк(max), где Aвых — работа выхода, Eк(max) — максимальная кинетическая энергия фотоэлектрона.
• Красная граница фотоэффекта — минимальная частота (максимальная длина волны), при которой фотоэффект ещё возможен: ν₀=Aвых/h.
## Орбитальный момент импульса электрона
• Момент импульса L=r×p электрона на орбите относительно центра — орбитальный момент импульса (механическая величина, не путать с пропорциональным ей магнитным моментом).
• Постулат Бора: момент импульса электрона на стационарной орбите кратен ħ=h/2π: L=nħ.` },
        ],
      },
    ],
  },
